...
class Simulation():
    def __init__(self, seed):
        ...
        # interval between the UAVs updating position
        self.T_sample = 3  # [s]
        # for graph
...
##########################################################
    ...
    def Start(self):
        ...
        Mountain_pos = [self.SCREEN_WIDTH / 2,
                        self.HEIGHT_game / 2 + self.HEIGHT_controller]
        # position of the mountain
        mountain_pos = (self.SCREEN_WIDTH / 2 - mountain_size / 2,
                        self.HEIGHT_game / 2 - mountain_size / 2 + self.HEIGHT_controller)
        self.Mountain_pos = (self.SCREEN_WIDTH / 2, self.HEIGHT_game / 2)
        # mountain radius
        self.mountain_radius = mountain_size / 2
        ...
        # dimension of missing person image
        h_d = disperso_img.get_height()
        w_d = disperso_img.get_width()

        # random coordinate for the missing
        self.coord_disperso = []
        for i in range(1, N_missing + 1):
            # distance between the missing person and the mountain
            dist_coord = -1
            while (dist_coord <= self.mountain_radius + 200):
                if i%2 == 0:
                    x_disp = random.randint(20, self.SCREEN_WIDTH/2 - w_d)
                    y_disp = random.randint(10, self.HEIGHT_game - h_d)
                else:
                    x_disp = random.randint(self.SCREEN_WIDTH/2 + self.mountain_radius, self.SCREEN_WIDTH-w_d)
                    y_disp = random.randint(self.HEIGHT_game/2 + h_d, self.HEIGHT_game - h_d )
        ...
        SF_display = pygame.Surface((SF_w + 10, 2 * SF_h + 10), pygame.SRCALPHA)
        SF_display.fill((80, 80, 80, 128))

        # Scaling factor width
        ...
        # right: Arrow points for angle = 0°
        ...
        circle_radius = drone_size * 15 / 100
        circle_radius1 = relay_size * 15 / 100
        ...
        # to enable the simulation sounds
        if SOUND:
            # loading sound #
        ...
        # division of the map according to the number of UAVs
        self.AreaDivision()
        ...
        while self.running:
            ...
            while (not self.FES.FES.empty()) and len(self.UAVs) > 0:
                ...
                for cloud_cl in self.clouds:
                    ...
                    # debugging
                    if self.FLAG_debug:
                        # it represents the circle on the cloud
                ...
                if self.FLAG_debug:
                    ...
                    # circle
                    pygame.draw.circle(screen, (0, 0, 155), (Mountain_pos[0], Mountain_pos[1]), self.mountain_radius,5)
                ...
                for drone in self.UAVs:
                    ...
                    # show the circle representing the bitrate status
                    ...
                    if self.FLAG_debug:
                        ...
                        # starting position of the lines
                        Start_position_ = [drone.x, self.HEIGHT_controller + drone.y]

                        # end positions of the lines
                ...
                # debugging: link between the relay and the ground station
                if self.FLAG_debug:
                    # length of the line
                    r = self.SCREEN_WIDTH / 2
                ...
                # changing of the color of the wind array according to the wind intensity
                # right wind
                ...
                # debugging: representation of the UAV velocity
                if FLAG_debug:
                    for drone in self.UAVs:
                        # effective direction
            ...
            # final metrics of the simulation
    ...
    def EventLoop(self):
        ...
        match self.event_type:
            ...
            # coordination or image send to the Ground Station. When missing person is found
            ...
            case 'Transmission attempt':
                ...
                # UAVs
                    ...
                    if index < len(self.UAVs):
                        # case : bitrate null. It sends packet to the relay
                        ...
                        # case : bitrate not null. It sends packet to the GS
                        else:
                            ...
                            self.Communication.channel_communication(self.UAVs[index], self.relay, self.sim_time,
                                                                     self.delta_msg_varying)
            # for multiple UAVs transmitting to the relay
            ...
            # when relay receives the packet
            ...
            # when the gs received the packet
            case 'Received Data':
                # UAV that has transmitted the packet
    ...
    def Choice_of_the_trajectory(self, id_drone):
        ...
        for drone in self.UAVs:
            ...
            # for a specific UAV
                ...
                # not ardupilot
                if not ARDUPILOT:
                    # we do not pass the position
                    pos = None
                # if there are waypoint yet it takes the successive one
    ...
    def update_UAV_location(self, time_interval):
        ...
        # evaluate the current Bit_Rate in function of the distance for the relay
        self.relay.drone.bitrate_fun(SCALE_FACTOR)

        # obstacles effect on the bit rate for the relay#
        for cloud_each in self.clouds:
        ...
        for drone in self.UAVs:
            ...
            if drone.tot_iterazioni > 0:
                ...
                # account the obstacle for the bit-rate. The effects of the clouds accumulate
                ...
                # update the bit rate status in the map
                drone.circle_color_fun()

                # find missing person procedure
                # center of the UAV
                drone_center = (drone.x + self.w_drone / 2, drone.y + self.h_drone / 2)

                cnt = 0
                for coord_missing in self.coord_disperso:
                    # distance between the center of the drone and the center of the missing
                    distance = math.sqrt((drone_center[0] - coord_missing[0]) ** 2 +
                                         (drone_center[1] - coord_missing[1]) ** 2)

                    # the missing person and the UAV are in range of visibility and the UAV does not send the same image twice
                    if distance <= UAV_VISIBLE_RANGE and cnt != self.missing_person_found_flag:
                        # just entered in the visibility range
                        if drone.found_missing_flag[cnt] == 0:
                            # add the event
                            self.FES.add_events(self.sim_time, 'Send Rescues Datas', (drone.id, cnt))
                            # number of missing found
            ...
            # destination reached
                ...
                # schedule a new event: next waypoint
            ...
            # when the battery is exausted, remove the UAV from the list
            if not drone.FLAG_Interrupt:
                ...
                if drone.id > 0:
                    ...
                    # to ArduPilot, it saves the waypoints and the velocity
                    if drone.id == 1:
    ...
    def Change_environmental_effects(self):
        ...
        # update the  effective velocity of the UAV relay
        (T, w, wind_angle, flag_pos) = self.check_position(self.relay.drone.x)
        self.relay.veloc_real(w, wind_angle, self.Mountain_pos[0], self.Mountain_pos[1])
        ...
        if add_or_rem == 1:  # add n clouds
            ...
            n_to_add_or_remove = random.randint(0, 2)
            ...
            for _ in range(n_to_add_or_remove):
                if len(self.clouds) > 0:
                    cloud_to_be_removed = random.choice(self.clouds)  # choose a random cloud from the list
                    self.clouds.remove(cloud_to_be_removed)  # remove

    def bitrate_null_Relay_UAV(self, drone, Mountain_pos, cloud_x, cloud_y):
        # useful for calculating the bitrate between UAV and relay
        ...
        for cloud_each in self.clouds:
            ...
            # coefficient considering the obstacle impact
    ...
    def check_position(self, x):
        # it checks the position wrt the mountain. Depending on the position the UAV is subject to different environmental conditions
    ...
    def Sampling(self):
        # saving the variables for the graphs in the dataframe
        for drone in self.UAVs:
            (T, w, wind_angle, flag_pos) = self.check_position(drone.x)
            # saving of the new row
            new_row = [self.sim_time, drone.id, T, self.T_amb2, self.T_amb, w, wind_angle * 180 / math.pi, drone.x,
                       drone.y, drone.mod_v_real,
                       drone.Ej / self.Initial_Ej * 100, drone.distance, drone.distance_GS,
                       drone.bitrate / 1e6]
            # we check that the type is not a list
    ...
    def graphs(self):
        ...
        if self.FLAG_graphs:
            ...
            # if the number of UAVs is less then 3 --> plot each UAVs statistics
            if self.N_UAV <= 3:
                ...
                for col in columns_:
                    ...
                    for id in IDs:  # one curve for each UAV
            ...
            # more than 3 UAVs: min, max and mean statistics
            else:
                ...
                for col in list_name:
                    ...
                    if i == 3:
                        ...
                        if columns_[j] in ['UAV speed [m/s]', 'battery level [%]', 'bitrate [Mbps]']:
                        ...
                        # end of simulation: vertical lines
                        ...
                        # the other columns are all equally treated
                        else:
    ...
    def text_controller(self, screen):
        ...
        # position for showing the environmental parameters
        X1 = self.SCREEN_WIDTH / 4 + 45
        X2 = 3 / 4 * self.SCREEN_WIDTH + 25

        # color of the wind arrays
